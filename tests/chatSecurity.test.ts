...
describe("Issue #7 — Lock Down Chat HTTP + Socket.IO", () => {
  ...
  beforeEach(() => {
    ...
    (prisma.worker.findUnique as jest.Mock).mockImplementation(({ where }: any) => {
      if (where.id === WORKER_A_ID) return Promise.resolve({ id: WORKER_A_ID, phone: "+919876543212", deleted_at: null });
      if (where.id === WORKER_B_ID) return Promise.resolve({ id: WORKER_B_ID, phone: "+919876543213", deleted_at: null });
  ...
  // =========================================================================
  describe("1. HTTP Authentication & Authorization", () => {
    ...
    it("Customer A CAN retrieve own chat history (200 OK)", async () => {
    ...
    it("Worker A (assigned) CAN retrieve chat history (200 OK)", async () => {
      (prisma.booking.findFirst as jest.Mock).mockResolvedValue({
        id: BOOKING_A_ID,
        customer_id: CUSTOMER_A_ID,
        worker_id: WORKER_A_ID,
    ...
    it("Worker B (unrelated) CANNOT retrieve Worker A's chat history (403/404 IDOR protection)", async () => {
      (prisma.booking.findFirst as jest.Mock).mockResolvedValue(null);
      (prisma.booking.findUnique as jest.Mock).mockResolvedValue({
        id: BOOKING_A_ID,
        customer_id: CUSTOMER_A_ID,
    ...
    it("Platform Admin CAN retrieve chat history (200 OK)", async () => {
      (prisma.booking.findFirst as jest.Mock).mockResolvedValue({
        id: BOOKING_A_ID,
        customer_id: CUSTOMER_A_ID,
        worker_id: WORKER_A_ID,
  ...
  // =========================================================================
  describe("3. Socket.IO Room Joining & Messaging Authorization", () => {
    ...
    it("Customer A CAN join booking room (join:booking)", (done) => {
      (prisma.booking.findFirst as jest.Mock).mockResolvedValue({
        id: BOOKING_A_ID,
        customer_id: CUSTOMER_A_ID,
        worker_id: WORKER_A_ID,
    ...
    it("Worker A (assigned) CAN join booking room (join:booking)", (done) => {
      (prisma.booking.findFirst as jest.Mock).mockResolvedValue({
        id: BOOKING_A_ID,
        customer_id: CUSTOMER_A_ID,
        worker_id: WORKER_A_ID,
    ...
    it("Customer B CANNOT join Customer A's booking room (FORBIDDEN, not joined)", (done) => {
      (prisma.booking.findFirst as jest.Mock).mockResolvedValue(null);
      (prisma.booking.findUnique as jest.Mock).mockResolvedValue({
        id: BOOKING_A_ID,
        customer_id: CUSTOMER_A_ID,
    ...
    it("Worker B CANNOT join Worker A's booking room (FORBIDDEN, not joined)", (done) => {
      (prisma.booking.findFirst as jest.Mock).mockResolvedValue(null);
      (prisma.booking.findUnique as jest.Mock).mockResolvedValue({
        id: BOOKING_A_ID,
        customer_id: CUSTOMER_A_ID,
