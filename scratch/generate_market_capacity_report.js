...
const content = `# LABOURBABA BACKEND — MARKET-CAPACITY CERTIFICATION & MAXIMUM SAFE LAUNCH ENVELOPE
...
- **Broadcast Delivery Verification:** 1 message sent from worker client $\rightarrow$ 1 message received by customer client.
...
const rootFile = path.resolve('LABOURBABA_MARKET_CAPACITY_CERTIFICATION_REPORT.md');
const brainFile = path.resolve('C:/Users/Divy/.gemini/antigravity-ide/brain/1ffaea16-62d0-4cfa-a0c2-5548118ade26/LABOURBABA_MARKET_CAPACITY_CERTIFICATION_REPORT.md');

fs.writeFileSync(rootFile, content, 'utf8');
console.log('Report written successfully to root:', rootFile);

try {
  fs.writeFileSync(brainFile, content, 'utf8');
  console.log('Report written successfully to brain:', brainFile);
} catch (e) {
  console.log('Note: could not write to brain:', e.message);
}
